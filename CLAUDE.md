...
- **Execution provider:** XNNPACK as default for FP32, CPU for INT8, NNAPI selectable via UI toggle (all three: CPU, XNNPACK, NNAPI). NNAPI uses `NNAPI_FLAG_CPU_DISABLED` on API 29+ to force NPU; falls back to XNNPACK/CPU on failure at both EP-append and session-creation stages (Exynos EDEN NPU rejects unsupported ops at session creation). Default startup mode: INT8 + CPU matcher (fastest measured: 80ms median on S21)
